...
مجلدات العمل العالمية
base_dir = os.path.expanduser("~")
shorts_dir = os.path.join(base_dir, "downloads", "shorts")

class SmartShortsHandler(http.server.SimpleHTTPRequestHandler):
def translate_path(self, path):
# توجيه الملفات الثابتة لتقرأ من مجلد shorts دائماً
root = shorts_dir
parsed = urllib.parse.urlparse(path)
path = parsed.path
path = os.urllib.parse.unquote(path) if hasattr(urllib.parse, 'unquote') else urllib.parse.unquote(path)
parts = path.split('/')
parts = [p for p in parts if p != '']
return os.path.join(root, *parts)

def do_GET(self):
# إذا دخل المستخدم للموقع لأول مرة ولم يتم توليد الفيديوهات بعد، نعرض صفحة الإعدادات
...
# 1. نظام المعالجة واستخراج الفيديوهات من الرابط
if self.path == '/process':
url = params.get('url', '').strip()
api_key = params.get('apiKey', '').strip()
try: clip_duration = int(params.get('duration', 30))
except: clip_duration = 30
try: max_clips = int(params.get('maxClips', 3))
except: max_clips = 3
...
fs_num = int(params.get('fontSize', 26)) # أصبح الآن رقم مباشر من السلايدر
...
global shorts_dir
if os.path.exists(shorts_dir): shutil.rmtree(shorts_dir)
...
if not os.path.exists(temp_clip): continue
...
// تحديث حجم الخط مباشرة عبر السلايدر
...
statusOutput.innerText = ⏳ Exporting Clip${clipSelector.value} with Custom Colors...; try { const response = await fetch('/export', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ clipId: clipSelector.value, style: styleSelector.value, position: captionLine.value, textColor: textColorSelector.value, bgColor: bgColorSelector.value, fontSize: fontSizeSlider.value }) }); const resData = await response.json(); if(resData.status === "success") { statusOutput.innerHTML =✅ Baked with Custom Colors!<br><a href="${resData.video}" download style="color:#fff; font-weight:bold; text-decoration:underline;">📥 Download Video</a>`;
}
...
global shorts_dir
# التأكد من تهيئة مجلد الإخراج الأساسي فقط في البداية دون مسح محتوياته في حالة الخادم يعمل
os.makedirs(shorts_dir, exist_ok=True)
